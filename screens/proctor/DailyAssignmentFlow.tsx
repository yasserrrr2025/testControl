...
interface Props {
  user: User;
  users: User[]; // تم التأكد من وجود قائمة المستخدمين لاختيار المعلم
...
const ProctorDailyAssignmentFlow: React.FC<Props> = ({ 
  ...
  const joinCommittee = async (committeeNum: string) => {
    ...
    try {
      ...
      onAlert(`تم المباشرة بنجاح في اللجنة ${cleanedNum}.`, 'success');
  ...
  const filteredTeachers = useMemo(() => {
    return users.filter(u => u.full_name.includes(teacherSearchTerm) || u.national_id.includes(teacherSearchTerm)).slice(0, 10);
  }, [users, teacherSearchTerm]);

  if (isInitialLoading) {
    return <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 animate-pulse text-slate-400"><Loader2 size={64} className="animate-spin text-blue-600" /><p className="font-black text-xl italic text-slate-500">جاري مزامنة بيانات اللجنة...</p></div>;
  }
    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto text-right pb-48 px-4 md:px-0">
       ...
       {committeeRequests.length > 0 && (
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-2 border-slate-50 space-y-4">
             <div className="flex items-center justify-between px-4 border-b pb-3 border-slate-100">
                <h4 className="text-sm font-black text-slate-800 flex items-center gap-3"><Clock size={20} className="text-blue-600"/> تتبع بلاغات اللجنة ({committeeRequests.length})</h4>
             </div>
             <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                {committeeRequests.map(req => (
                   <div key={req.id} className="min-w-[300px] bg-slate-50 border border-slate-100 p-6 rounded-3xl flex flex-col gap-4 relative overflow-hidden group shadow-sm transition-all hover:bg-white hover:shadow-md">
                      <div className={`absolute top-0 right-0 w-2 h-full ${req.status === 'DONE' ? 'bg-emerald-500' : req.status === 'IN_PROGRESS' ? 'bg-blue-500 animate-pulse' : 'bg-red-500'}`}></div>
                      <div className="flex justify-between items-center pr-2">
                         <span className="text-[10px] font-black uppercase text-slate-400 font-mono">{new Date(req.time).toLocaleTimeString('ar-SA', {hour:'2-digit', minute:'2-digit'})}</span>
                         <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase ${req.status === 'DONE' ? 'bg-emerald-100 text-emerald-600' : req.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>
                            {req.status === 'DONE' ? 'تم التنفيذ' : req.status === 'IN_PROGRESS' ? 'جاري المباشرة' : 'قيد الانتظار'}
                         </span>
                      </div>
                      <p className="text-sm font-black text-slate-800 leading-relaxed truncate pr-2">{req.text}</p>
                      {req.assistant_name && (
                        <div className="flex items-center gap-2 border-t pt-3 mt-2 pr-2 text-slate-400">
                           <UserCheck size={14} className="text-blue-500"/>
                           <span className="text-[10px] font-black">بواسطة: {req.assistant_name}</span>
                        </div>
                      )}
                   </div>
                ))}
           ...
           return (
             <div key={s.id} className={`p-10 rounded-[4rem] border-2 transition-all relative overflow-hidden flex flex-col justify-between shadow-2xl min-h-[380px] ${status?.type === 'ABSENT' ? 'bg-red-50/70 border-red-200 shadow-red-100' : status?.type === 'LATE' ? 'bg-amber-50/70 border-amber-200 shadow-amber-100' : 'bg-white border-slate-50 hover:border-blue-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mt-10">
                   ...
                   <button onClick={() => toggleStudentStatus(s, 'LATE')} className={`py-6 rounded-[2rem] font-black text-xs transition-all shadow-lg active:scale-95 ${status?.type === 'LATE' ? 'bg-amber-500 text-white' : 'bg-white text-slate-400 hover:bg-amber-50 border-2 border-slate-50'}`}>رصد تأخر</button>
                </div>
             </div>
           );
         })}
                   ...
                   {requestSubView === 'SELECT_STUDENTS' ? (
                     <div className="space-y-10 animate-fade-in">
                        <div className="text-center">
                           <h4 className="text-3xl font-black text-slate-800 tracking-tight">اختيار الطلاب المستحقين</h4>
                        ...
                        <div className="max-h-[400px] overflow-y-auto space-y-4 custom-scrollbar px-2">
                           {myStudents.map(s => (
                             <button 
                                ...
                                className={`w-full p-8 rounded-[3rem] border-2 transition-all text-right flex justify-between items-center ${selectedStudentsForReq.includes(s.national_id) ? 'border-red-600 bg-red-50 shadow-2xl' : 'border-slate-100 bg-slate-50'}`}
                             >
                                <div className="text-right">
                                   <span className="font-black text-2xl text-slate-800 block leading-tight">{s.name}</span>
                                   <span className="text-xs text-slate-400 font-bold mt-1">رقم الجلوس: {s.seating_number || '---'}</span>
                                </div>
                        ...
                        <button 
                          onClick={() => { 
                             ...
                             sendFinalRequest(`${currentRequestLabel} لـ: ${names}`); 
                          }} 
                          disabled={selectedStudentsForReq.length === 0} 
                          className="w-full bg-red-600 text-white py-10 rounded-[2.5rem] font-black text-3xl shadow-2xl disabled:opacity-50 active:scale-95 border-b-[8px] border-red-800"
                        >
                     ...
                     <div className="space-y-8 animate-fade-in">
                        ...
                        <div className="max-h-[400px] overflow-y-auto custom-scrollbar space-y-3 px-2">
                           {filteredTeachers.map(t => (
                             <button 
                               key={t.id} 
                               onClick={() => sendFinalRequest(`${currentRequestLabel}: ${t.full_name}`)}
                               className="w-full p-8 rounded-[3rem] border-2 bg-slate-50 border-slate-100 text-right font-black text-2xl hover:border-indigo-600 hover:bg-white transition-all shadow-md group"
                             >
                           ...
                           {filteredTeachers.length === 0 && (
                             <div className="py-10 text-center text-slate-300 font-bold italic">لا يوجد نتائج لهذا البحث</div>
                           )}
                        </div>
                     </div>
                     ...
                     <div className="text-center space-y-12 animate-fade-in">
                        ...
                        <button onClick={() => sendFinalRequest(`${currentRequestLabel} (العدد: ${requestCount})`)} className="w-full bg-red-600 text-white py-10 rounded-[2.5rem] font-black text-3xl shadow-2xl active:scale-95 border-b-[8px] border-red-800">تأكيد وإرسال البلاغ</button>
                     </div>
                      ...
                      <div className="space-y-10 animate-fade-in">
                         <textarea 
                           ...
                           placeholder="اكتب بلاغك هنا بوضوح..." 
                           className="w-full bg-slate-50 border-4 border-slate-100 rounded-[3rem] p-12 font-bold text-3xl h-72 outline-none focus:border-red-600 transition-all text-right shadow-inner resize-none" 
                         />
                         <button onClick={() => sendFinalRequest(`بلاغ مخصص: ${customMessage}`)} disabled={!customMessage.trim()} className="w-full bg-red-600 text-white py-10 rounded-[2.5rem] font-black text-3xl shadow-2xl disabled:opacity-50 active:scale-95 border-b-[8px] border-red-800">إرسال البلاغ المكتوب</button>
                      </div>
                   ) : (
                     <div className="grid grid-cols-2 gap-6">
                        {[
                          { icon: PackageSearch, label: 'نقص ورق إجابة', type: 'STUDENTS', color: 'text-blue-600' },
                          ...
                          { icon: FileWarning, label: 'نقص ورق أسئلة', type: 'COUNT', color: 'text-rose-600' },
                          { icon: UserSearch, label: 'طلب معلم المادة', type: 'TEACHER', color: 'text-indigo-600' },
                          { icon: Backpack, label: 'طلب مرسام', color: 'text-slate-600' },
                          { icon: MessageCircle, label: 'بلاغ آخر (كتابة)', type: 'CUSTOM', color: 'text-slate-400' }
                        ].map((item, idx) => (
                          <button key={idx} onClick={() => handleQuickRequest(item.label, item.type as any)} className="p-12 bg-slate-50 border-2 border-slate-100 rounded-[4rem] flex flex-col items-center gap-6 hover:border-red-300 transition-all group hover:bg-white active:scale-95 shadow-lg">
                             <item.icon size={64} className={`${item.color} group-hover:scale-110 transition-transform`} />
