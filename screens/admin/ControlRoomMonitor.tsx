...
const ControlRoomMonitor: React.FC<Props> = ({ absences, supervisions, users, deliveryLogs, students, requests }) => {
  ...
  const committeeGrid = useMemo(() => {
    ...
    return comNums.map(num => {
      ...
      // حالة "متجه للكنترول": المراقب سلم الميداني (PENDING) ولكن الكنترول لم يعتمد بعد
  ...
  // مكون فرعي لعرض خريطة اللجان
  const MapPanel = ({ isMaximized = false }) => (
    <div className={`${isMaximized ? 'fixed inset-0 z-[1000] bg-[#020617] p-10' : 'bg-white/[0.02] border border-white/5 rounded-[3.5rem] p-8 flex flex-col h-[55%] shadow-inner'} relative overflow-hidden group transition-all duration-500`}>
      <div className="absolute -top-24 -left-24 w-64 h-64 bg-blue-600/5 blur-[100px] rounded-full"></div>
      <div className="flex items-center justify-between mb-8 relative z-10">
         <div className="flex items-center gap-4">
            <div className="bg-blue-600/10 p-4 rounded-[1.5rem] text-blue-400 shadow-inner"><LayoutGrid size={isMaximized ? 40 : 28} /></div>
            <div>
              <h2 className={`${isMaximized ? 'text-5xl' : 'text-3xl'} font-black tracking-tighter`}>خريطة اللجان الحية</h2>
         ...
         <div className="flex gap-6 items-center">
            <div className="flex gap-6 items-center bg-black/40 px-8 py-3 rounded-full border border-white/5 text-[9px] font-black uppercase tracking-widest">
                ...
                <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-orange-500 animate-pulse"></div><span>متجه للكنترول</span></div>
            ...
            <button onClick={() => toggleMaximize('MAP')} className="p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 relative z-10">
         <div className={`grid ${isMaximized ? 'grid-cols-8 md:grid-cols-10 lg:grid-cols-12 xl:grid-cols-15' : 'grid-cols-6 md:grid-cols-8 lg:grid-cols-10'} gap-4`}>
            {committeeGrid.map(c => (
              <div key={c.num} className={`
                ...
                ${c.isDone ? 'bg-emerald-600 border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 
                  ...
                  c.isSubmitted ? 'bg-orange-500 border-orange-300 shadow-[0_0_30px_rgba(249,115,22,0.4)] animate-pulse scale-105' :
              ...
              `}>
                ...
                <span className={`${isMaximized ? 'text-4xl' : 'text-3xl'} font-black tabular-nums tracking-tighter leading-none`}>{c.num}</span>
                {c.isSubmitted && !c.isDone && <MoveRight size={14} className="absolute bottom-2 animate-bounce-right text-white/50" />}
                {c.isOccupied && !c.isDone && !c.isSubmitted && <div className="absolute bottom-3 w-1.5 h-1.5 bg-blue-400 rounded-full animate-ping"></div>}
              </div>
            ))}
         </div>
      </div>
  ...
  // مكون فرعي لعرض جدول الغياب
  const AbsencesPanel = ({ isMaximized = false }) => (
    <div className={`${isMaximized ? 'fixed inset-0 z-[1000] bg-[#020617] p-10' : 'bg-white/[0.03] border border-white/10 rounded-[3.5rem] p-10 flex flex-col h-[45%] shadow-2xl border-b-[8px] border-blue-600'} relative overflow-hidden transition-all duration-500`}>
       <div className="absolute top-0 right-0 w-48 h-1 bg-blue-500 shadow-[0_0_30px_rgba(37,99,235,1)]"></div>
       
       <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
             <div className="p-4 bg-blue-600/10 text-blue-400 rounded-2xl"><Users size={isMaximized ? 40 : 28} /></div>
             <div>
                <h3 className={`${isMaximized ? 'text-5xl' : 'text-2xl'} font-black text-white tracking-tight`}>بيانات غياب وتأخر اللجان</h3>
          ...
          <div className="flex gap-4">
             ...
             <button onClick={() => toggleMaximize('ABSENCES')} className="p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all">
                {isMaximized ? <Minimize2 size={24}/> : <Maximize2 size={24}/>}
             </button>
          </div>
       </div>
       ...
       <div className="flex-1 overflow-y-auto custom-scrollbar">
          <table className="w-full text-right border-collapse">
             <thead className={`sticky top-0 bg-[#020617] ${isMaximized ? 'text-sm' : 'text-[11px]'} font-black text-slate-500 uppercase tracking-widest border-b border-white/10`}>
                <tr>
             ...
             <tbody className="divide-y divide-white/5">
                {absences.length === 0 ? (
                  <tr><td colSpan={5} className="py-16 text-center text-slate-700 font-black italic text-xl opacity-20 tracking-widest">الميدان الدراسي مستقر تماماً</td></tr>
                ) : (
                  absences.sort((a,b) => b.date.localeCompare(a.date)).map(a => {
                    const student = students.find(s => s.national_id === a.student_id);
                    return (
                      <tr key={a.id} className={`hover:bg-white/[0.02] transition-colors group ${isMaximized ? 'text-lg' : 'text-base'}`}>
                         <td className="py-5 px-6">
                            <div className="flex items-center gap-4">
                               ...
                               <span className="font-black text-white group-hover:text-blue-400 transition-colors">{a.student_name}</span>
                            </div>
                         </td>
                         <td className="py-5 px-6">
  ...
  // مكون فرعي لعرض البلاغات
  const ReportsPanel = ({ isMaximized = false }) => (
    <div className={`${isMaximized ? 'fixed inset-0 z-[1000] bg-[#020617] p-10' : 'bg-white/[0.03] border border-white/10 rounded-[3.5rem] p-8 flex flex-col flex-1 overflow-hidden shadow-2xl'} relative transition-all duration-500`}>
       ...
       <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4 relative z-10">
          <div className="flex items-center gap-3">
             <ShieldAlert size={isMaximized ? 40 : 24} className="text-red-500 animate-pulse" />
             <h2 className={`${isMaximized ? 'text-4xl' : 'text-xl'} font-black text-white tracking-tighter`}>بلاغات العمليات</h2>
          </div>
          <div className="flex items-center gap-4">
             ...
             <button onClick={() => toggleMaximize('REPORTS')} className="p-4 bg-white/5 hover:bg-white/10 rounded-2xl transition-all">
                {isMaximized ? <Minimize2 size={24}/> : <Maximize2 size={24}/>}
             </button>
          </div>
       </div>
                ...
                `}>
                   ...
                   <div className="flex justify-between items-start mb-4 pt-2">
                      <div className="flex items-center gap-4">
                         ...
                         <div>
                            <p className={`${isMaximized ? 'text-xl' : 'text-sm'} font-black text-white group-hover:text-blue-400 transition-colors leading-tight mb-1`}>{req.from}</p>
                   ...
                   <div className={`p-5 bg-black/40 rounded-[1.8rem] border border-white/5 shadow-inner`}>
                      <p className={`${isMaximized ? 'text-2xl' : 'text-base'} font-black text-white leading-relaxed`}>{req.text}</p>
                   </div>
                </div>
              );
            })
  ...
  return (
    <div className="fixed inset-0 bg-[#020617] text-white overflow-hidden font-['Tajawal'] z-[100] flex flex-col p-4 dir-rtl text-right">
      ...
      {/* الرندرة الشرطية للتكبير */}
      {maximizedPanel === 'MAP' && <MapPanel isMaximized />}
      {maximizedPanel === 'ABSENCES' && <AbsencesPanel isMaximized />}
      {maximizedPanel === 'REPORTS' && <ReportsPanel isMaximized />}
